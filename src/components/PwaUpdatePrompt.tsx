...
export function PwaUpdatePrompt({ onUpdate }: { onUpdate: () => void }) {
  ...
  return (
    <div
      className="fixed top-4 left-1/2 z-[90] w-[min(94vw,22rem)] -translate-x-1/2 rounded-xl border border-brand-700/40 bg-zinc-900/95 backdrop-blur-xl px-4 py-3 shadow-xl safe-top"
      role="status"
      aria-live="polite"
    >
      <div className="flex items-center gap-3">
        <RefreshCw className="w-4 h-4 text-brand-400 shrink-0" aria-hidden="true" />
        <p className="flex-1 text-xs sm:text-sm text-zinc-200">A new version is ready.</p>
        <button
          type="button"
          onClick={() => {
            onUpdate();
